...
def times(sta):
    ...
    text_sur = text_go.render(f" {((currently - stra_time ))} ",False,'black').convert_alpha()
...
def obs_mov(obs_list) :
    if obs_list:
       for obj_rec in obs_list:
           obj_rec.x -= 7 
        
           screen.blit(turt,obj_rec)
       return obs_list
    else : return []   

def dell(obj_lis):
    for obj in obj_lis:
        if obj.centerx <0 or not game_act:
            obj_lis.pop(0) 
...
gumba = pygame.image.load('gumba.png')
gumba = pygame.transform.rotozoom(gumba ,0,0.5)
gumba_rec = gumba.get_rect(midbottom = (1200,420))
...
while True:  
    for even in pygame.event.get():
        ...
        if even.type == obs_tim and game_act:
            ...
            obj_list.append(enemy.get_rect(bottomright =( np.random.randint(900,1200),420)))
            if len(obj_list) >7 :
                obj_list.pop(0)
